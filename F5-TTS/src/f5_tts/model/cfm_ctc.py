...
class CFM_CTC(nn.Module):
    ...
    def sample(
        ...
        *,
        ...
        # [Modified] Branching fn definition based on self.CA
        if self.CA:
            def fn(t, x):
                ...
                if cfg_strength < 1e-5:
                    pred, _ = self.transformer(
                        ...
                        time=t,
                        mask=mask,
                        text_mask=text_mask, # Pass text_mask
        ...
        *,
        ...
        if self.CA:
            pred, intermediates_ctc = self.transformer(
                x=φ, cond=cond, text=text, time=time, drop_audio_cond=drop_audio_cond, drop_text=drop_text, mask=mask, text_mask=text_mask
            )
        else:
            pred, intermediates_ctc = self.transformer(
