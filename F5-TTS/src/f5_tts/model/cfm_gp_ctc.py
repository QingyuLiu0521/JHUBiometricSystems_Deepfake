...
class CFM_GP_CTC(nn.Module):
    ...
    def forward(
        ...
        text: int["b nt"] | list[str],
        total_text: int["b nt"] | list[str],
        *,
        lens: int["b"] | None = None,
        total_text_lens: int["b"] | None = None,
        ...
        if isinstance(text, list):
            if exists(self.vocab_char_map):
                text = list_str_to_idx(text, self.vocab_char_map).to(device)
                total_text = list_str_to_idx(total_text, self.vocab_char_map).to(device)
            else:
                text = bpe_padded(text).to(device)
                total_text = bpe_padded(total_text).to(device)
            assert text.shape[0] == batch
        ...
        if self.ctc_lambda > 0:
            ...
            for intermediate in intermediates_ctc.values():
                ...
                ctc_loss += F.ctc_loss(
                    ...
                    total_text,  # Target sequences (b nt)
                    z_lens,  # Length of inputs
                    total_text_lens,  # Length of targets
