...
# from f5_tts.infer.utils_infer import load_checkpoint, cfg_strength, nfe_step, sway_sampling_coef
nfe_step = 32  # 16, 32
cfg_strength = 2.0
...
class Inferencer_gp:
    ...
    def inference(self, dataset: Dataset, num_workers=16, resumable_with_seed: int = 666):
        # 设置 Generator 以确保 Shuffle/Batching 的可复现性（如果需要）
        ...
        # dtype = torch.float32 if self.mel_spec_type == "bigvgan" else None
        # self.model = load_checkpoint(self.model, self.checkpoint_path, str(self.accelerator.device), dtype=dtype, use_ema=True)
        # self.model.eval()
    ...
    def process_batch(
        ...
        text: int["b nt"] | list[str],  # noqa: F722 原始句子，文本形式/未处理的字符串列表
        ...
        # 创建shuffled text
        if batch > 1:
            shuffle_indices = create_derangement(batch)
            text_changed = [text[i] for i in shuffle_indices]
            text_changed = trim_text(text_changed, frac_lengths)
        else:
            random_text = choice(available_texts)
            text_changed = [random_text]
        
        # 构建source model的输入文本：原始text + " " + shuffled text
        text_source_input = []
        durations = []
