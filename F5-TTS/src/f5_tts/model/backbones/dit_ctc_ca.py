...
class InputEmbedding_AudioOnly(nn.Module):
...
class DiT_CTC_CA(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.input_embed = InputEmbedding_AudioOnly(mel_dim, dim)
    ...
    def get_input_embed(
        ...
        if self.text_uncond is None or self.text_cond is None or not cache:
            text_embed = self.text_embed(text, text.shape[1], drop_text=drop_text)
            if cache:
                if drop_text:
        ...
        x = self.input_embed(x, cond, drop_audio_cond=drop_audio_cond, audio_mask=audio_mask)
    ...
    def forward(
        ...
        if cfg_infer:  # pack cond & uncond forward: b n d -> 2b n d
            ...
            text_embed = torch.cat([text_embed_cond, text_embed_uncond], dim=0) # 新增
            mask = torch.cat((mask, mask), dim=0) if mask is not None else None
            text_mask = torch.cat((text_mask, text_mask), dim=0) if text_mask is not None else None # 新增
        else:
        ...
        for layer_i, block in enumerate(self.transformer_blocks):
            if self.checkpoint_activations:
                ...
                x = block(
                    x,
                    t,
                    mask=mask, # mask=None if self.training else mask,
                    rope=rope,
                    text=text_embed,
                    text_mask=text_mask,
                )

            if not cache and layer_i in self.layer_map_ctc:
