...
class TextEmbedding(nn.Module):
    def __init__(
        ...
        self.isomorphic_alignment = isomorphic_alignment
        self.prompt_token_enabled = prompt_token_enabled
    ...
    def average_upsample_text_isomorphic(self, text, text_mask, target_lens, prompt_lens):
        batch, _, text_dim = text.shape
        text_lens = text_mask.sum(dim=1)  # [batch]
        upsampled_text = torch.zeros_like(text)

        prompt_lens = prompt_lens.to(device=text.device, dtype=torch.long)
        target_lens = target_lens.to(device=text.device, dtype=torch.long)
        ...
        for i in range(batch):
            ...
            prompt_len = max(0, min(prompt_len, audio_len))
            valid_ind = torch.where(text_mask[i])[0]
            if valid_ind.numel() == 0:
                continue

            target_audio_len = audio_len - prompt_len
            if target_audio_len <= 0:
                continue

            if self.prompt_token_enabled:
                # Plan B: token 0 is <prompt>, replicate it to audio prompt span.
                prompt_embed = text[i, valid_ind[0], :]
                if prompt_len > 0:
                    upsampled_text[i, :prompt_len, :] = prompt_embed.unsqueeze(0).expand(prompt_len, text_dim)

                # Remaining valid text tokens map to target speech span.
                target_valid_data = text[i, valid_ind[1:], :]
            else:
                # Plan A: keep prompt span as filler/zero, and map all valid text tokens to target span.
                target_valid_data = text[i, valid_ind, :]

            target_text_len = int(target_valid_data.shape[0])
            if target_text_len == 0:
                continue

            base_repeat = target_audio_len // target_text_len
            remainder = target_audio_len % target_text_len

            indices = []
            for j in range(target_text_len):
                repeat_count = base_repeat + (1 if j >= target_text_len - remainder else 0)
                indices.extend([j] * repeat_count)

            indices = torch.tensor(indices[:target_audio_len], device=text.device, dtype=torch.long)
            upsampled_target = target_valid_data[indices]
            upsampled_text[i, prompt_len : prompt_len + target_audio_len, :] = upsampled_target

        return upsampled_text

    def forward(self, text: int["b nt"], seq_len, drop_text=False, prompt_lens: int["b"] | None = None):
        ...
        if self.average_upsampling:
            ...
            if self.isomorphic_alignment and prompt_lens is not None:
                text = self.average_upsample_text_isomorphic(text, ~text_mask, target_lens, prompt_lens)
            else:
...
class DiT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.text_embed = TextEmbedding(
            ...
            prompt_token_enabled=prompt_token_enabled,
        )
    ...
    def get_input_embed(
        ...
        if self.text_uncond is None or self.text_cond is None or not cache:
            ...
            text_embed = self.text_embed(text, seq_len=seq_len, drop_text=drop_text, prompt_lens=prompt_lens)
            if cache:
                if drop_text:
    ...
    def forward(
        ...
        if cfg_infer:  # pack cond & uncond forward: b n d -> 2b n d
            x_cond = self.get_input_embed(
                x, cond, text, drop_audio_cond=False, drop_text=False, cache=cache, audio_mask=mask, prompt_lens=prompt_lens
            )
            x_uncond = self.get_input_embed(
                x, cond, text, drop_audio_cond=True, drop_text=True, cache=cache, audio_mask=mask, prompt_lens=prompt_lens
            )
