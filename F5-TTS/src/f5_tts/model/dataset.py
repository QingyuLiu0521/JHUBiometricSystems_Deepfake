...
class CustomDataset_gp_t(Dataset):
    def __init__(
        self,
        custom_dataset: Dataset,
        durations=None,
    ...
    def __getitem__(self, index):
        while True:
            ...
            # filter by given length
            if 0.3 <= duration <= 30 and total_frames <= 4096:
                break  # valid

            index = (index + 1) % len(self.data)

        prompt_mel = torch.load(mel_path, map_location='cpu', weights_only=True)
...
class CustomDataset_gp_t_ctc(Dataset):
    def __init__(
        self,
        custom_dataset: Dataset,
        durations=None,
...
def load_dataset_gp_t(
    ...
    layer_indices_ctc: list[int] | None = None,
) -> CustomDataset | HFDataset:
    """
    dataset_type    - "CustomDataset" if you want to use tokenizer name and default data path to load for train_dataset
                    - "CustomDatasetPath" if you just want to pass the full path to a preprocessed dataset without relying on tokenizer
    ...
    if dataset_type == "CustomDataset":
        if target_hour is None:
            ...
            rel_data_path = str(files("f5_tts").joinpath(f"../../data/{dataset_name}_{tokenizer}_gp_t_hours{target_hour}"))
        if audio_type == "raw":
            try:
                train_dataset = load_from_disk(f"{rel_data_path}/raw")
            except:  # noqa: E722
        ...
        if layer_indices_ctc is not None:
            train_dataset = CustomDataset_gp_t_ctc(
                train_dataset,
                durations=durations,
            ...
            train_dataset = CustomDataset_gp_t(
                train_dataset,
                durations=durations,
                prompt_frames=prompt_frames,
                preprocessed_mel=preprocessed_mel,
