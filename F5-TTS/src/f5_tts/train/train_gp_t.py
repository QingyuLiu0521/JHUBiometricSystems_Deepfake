...
from f5_tts.model import CFM_GP, Trainer_GP
from f5_tts.model.dataset import load_dataset_gp_t
from f5_tts.model.utils import get_tokenizer_gp_t

os.chdir(str(files("f5_tts").joinpath("../..")))  # change working directory to root of project (local editable)

@hydra.main(version_base="1.3", config_path=str(files("f5_tts").joinpath("configs")), config_name="F5TTS_v1_Base_debug_Emilia_gp_train.yaml")
...
def main(model_cfg):
    ...
    isomorphic_alignment = bool(model_arc.get("isomorphic_alignment", False))
    prompt_token_enabled = bool(model_arc.get("prompt_token_enabled", False))
    text_num_embeds = vocab_size + (1 if prompt_token_enabled else 0)
    ...
    model = CFM_GP(
        ...
        prompt_token_id=vocab_size if prompt_token_enabled else None,
    )
    ...
    train_dataset = load_dataset_gp_t(model_cfg.datasets.name, tokenizer, mel_spec_kwargs=model_cfg.model.mel_spec)
    trainer.train(
        train_dataset,
