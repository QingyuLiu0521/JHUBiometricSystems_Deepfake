...
def deal_with_audio_dir(audio_dir, gen_dir, dnsmos_filter):
    gen_dir_path = Path(gen_dir) / Path(*audio_dir.parts[-2:])
    audio_jsonl = gen_dir_path.with_suffix(".jsonl")
    ...
    with open(audio_jsonl, "r") as f:
        ...
        for line in tqdm(lines, desc=f"{audio_jsonl.stem}"):
            ...
            if tokenizer == "pinyin":
                ...
                gen_text = convert_char_to_pinyin([gen_text], polyphone=polyphone)[0]
            duration = obj["duration"]
            mel_path = str((gen_dir_path.parent / obj["wav"]).with_suffix(".pt"))
            prompt_frames = obj["gen_len"]
            sub_result.append({"audio_path": str(audio_dir.parent / obj["wav"]),
                               "text": text,
                               "duration": duration,
                               "mel_path": mel_path,
    ...
    return sub_result, durations, prompt_frames_list, vocab_set, bad_case_zh, bad_case_en, bad_case_dnsmos
...
def main():
    ...
    for lang in langs:
        ...
        [
            futures.append(executor.submit(deal_with_audio_dir, audio_dir, gen_dir, dnsmos_filter))
            for audio_dir in dataset_path.iterdir()
            if audio_dir.is_dir()
        ]
    for futures in tqdm(futures, total=len(futures)):
        sub_result, durations, prompt_frames, vocab_set, bad_case_zh, bad_case_en, bad_case_dnsmos = futures.result()
        ...
        total_bad_case_dnsmos += bad_case_dnsmos
    executor.shutdown()
    ...
    base_vocab_path = str(files("f5_tts").joinpath("../../")) + f"/data/Emilia_{'_'.join(langs)}_{tokenizer}_gp_t/vocab.txt"
    ...
    print(f"Bad dnsmos case (< {dnsmos_filter}): {total_bad_case_dnsmos}\n")
...
if __name__ == "__main__":
    ...
    dnsmos_filter = 3.4  # e.g. 3.3, 3.4, 3.35

    langs = ["ZH", "EN"]
    dataset_dir = "/vepfs/group09/speech_data/Emilia_data"
    gen_dir = "/vepfs/group09/F5-TTS_qingyu_MultilingualTTS/F5-TTS-main/Emilia_ZH_EN_gen"
    dnsmos_str = str(dnsmos_filter).replace(".", "_")
    dataset_name = f"Emilia_{'_'.join(langs)}_{tokenizer}_gp_t_{dnsmos_str}"
