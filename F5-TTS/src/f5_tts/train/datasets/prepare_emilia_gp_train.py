...
def deal_with_audio_dir(audio_dir, gen_dir):
    ...
    return sub_result, durations, prompt_frames_list, vocab_set, bad_case_zh, bad_case_en
...
def main():
    ...
    for lang in langs:
        ...
        [
            futures.append(executor.submit(deal_with_audio_dir, audio_dir, gen_dir))
            for audio_dir in dataset_path.iterdir()
            if audio_dir.is_dir()
        ]
    for futures in tqdm(futures, total=len(futures)):
        sub_result, durations, prompt_frames, vocab_set, bad_case_zh, bad_case_en = futures.result()
...
if __name__ == "__main__":
    ...
    dataset_name = f"Emilia_{'_'.join(langs)}_{tokenizer}_gp_t"
